...
class ParserTest:
  @Test def parse(): Unit =
    ...
    assertEquals(Right(JsonValue.Array(array)), Parser.parse(scanner))

  @Test def parse_empty(): Unit =
    val scanner = Scanner("")
    assertEquals(Right(JsonValue.Null), Parser.parse(scanner))

  @Test def parse_number(): Unit =
    val scanner = Scanner("-123.45")
    assertEquals(Right(JsonValue.Number(-123.45)), Parser.parse(scanner))

  @Test def parse_string(): Unit =
    val scanner = Scanner(""""abc"""")
    assertEquals(Right(JsonValue.String("abc")), Parser.parse(scanner))

  @Test def parse_empty_array(): Unit =
    val scanner = Scanner("[]")
    assertEquals(Right(JsonValue.Array(List())), Parser.parse(scanner))
  ...
  @Test def parse_empty_object(): Unit =
    val scanner = Scanner("{}")
    assertEquals(Right(JsonValue.Object(Map())), Parser.parse(scanner))
